...
export default function Dashboard({ mainRef, counterBalance, onReturnToLanding, onOpenAccountModal }: any) {
  ...
  // Build cards array — Approve always, OwnerTransferFrom only if owner
  const cards = [
  ...
  return (
    <main ref={mainRef} className="relative overflow-auto w-full h-full p-4 md:p-16 flex flex-col bg-[#020202] text-white">
      ...
      <header className="pr-[32px] mb-14 py-10 flex flex-col lg:flex-row justify-between items-center lg:items-center gap-12">
        {/* ... entire header remains 100% the same as your original ... */}
          ...
          <div className="relative flex-shrink-0 group cursor-pointer">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              ...
              <div 
                ...
                style={{ borderRadius: '50% 50% 20% 20%', animationDelay: '0.2s' }}
              />
            </div>
            ...
            <div className="relative z-10 w-24 h-24 md:w-32 md:h-32 flex items-center justify-center transition-transform duration-500 group-hover:scale-105">
              <img src="/logo2.png" className="w-2/3 h-auto drop-shadow-[0_0_8px_rgba(59,130,246,0.6)] relative z-20" alt="PepsiCoin Asset" />
            </div>
          </div>
      ...
      {/* GRID — now Approve and OwnerTransferFrom are separate items */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 relative z-10 pb-20" onMouseLeave={() => setHoveredIndex(null)}>
        ...
        {cards.map((card, idx) => (
          ...
          >
            ...
            <footer className="border-t border-white/5 bg-[#0a0a0a]/50 backdrop-blur-sm py-6 ">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
          <div className="flex items-center gap-4">
            <img src="/logo2.png" alt="PepsiCoin" className="w-12 h-10" />
            <div>
              <p className="text-white font-bold italic tracking-tight">PEPSI<span className="text-[#E30613]">COIN</span> © 2025</p>
              <p className="text-xs text-white/50 font-mono tracking-wider">PC-2025_INIT • PROTOCOL_ACTIVE</p>
            </div>
          </div>

          <p className="text-xs text-white/40">
            The future of meme finance. Powered by community.
          </p>
        </div>
      </footer>
